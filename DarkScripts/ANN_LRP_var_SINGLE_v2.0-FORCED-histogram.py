"""
Train the model on the different LENS-SINGLE runs to get slopes of ensembles
and remove the ensemble mean
    ...
    for seas in range(len(seasons)):
        ...
        for isample in range(SAMPLEQ): 
            ...
            if rm_annual_mean == True:
                directoryfigure = '/Users/zlabe/Desktop/SINGLE_v2.0/Histogram/rm_annual_mean/'
            
            ### Remove the meridional mean? True to subtract it from dataset ######
            rm_merid_mean = False #################################################
            if rm_merid_mean == True:
                directoryfigure = '/Users/zlabe/Desktop/SINGLE_v2.0/Histogram/rm_merid_mean/'
            
            ### Calculate only over land? True if land ############################
            land_only = False ######################################################
            if land_only == True:
                directoryfigure = '/Users/zlabe/Desktop/SINGLE_v2.0/Histogram/land_only/'
            
            ### Calculate only over ocean? True if ocean ##########################
            ocean_only = False #####################################################
            if ocean_only == True:
                directoryfigure = '/Users/zlabe/Desktop/SINGLE_v2.0/Histogram/ocean_only/'
            
            ### Rove the ensemble mean? True to subtract it from dataset ##########
            rm_ensemble_mean = True ##############################################
            if rm_ensemble_mean == True:
                directoryfigure = '/Users/zlabe/Desktop/SINGLE_v2.0/Histogram/rm_ensemble_mean/'
            ...
            ### Select testing ensembles to save
            testens = YpredTest.reshape(len(testIndices),
                                              len(years))
            actualens = (np.arange(np.shape(testens)[1]) + yearsall[sis].min())
            
            ### Regression of each ensemble in the testing 
            slopeens = np.empty((testens.shape[0]))
            interceptoens = np.empty((testens.shape[0]))
            r_valueens = np.empty((testens.shape[0]))
            p_valueens = np.empty((testens.shape[0]))
            std_errens = np.empty((testens.shape[0]))
            for e in range(testens.shape[0]):
                slopeens[e],interceptoens[e],r_valueens[e],p_valueens[e],std_errens[e] = stats.linregress(actualens[:],testens[e,:])
                
            slopeensall = np.mean(slopeens)
            r_valueensall = np.mean(r_valueens)
            
            ### Append slopes
            valslopes.append(slopeensall)
            valrr.append(r_valueensall)
...
np.savetxt(directorydataoutput + 'Slopes_1920-2080-FORCEDens_XGHG-XAER-LENS_%s_RANDOMSEED_20ens.txt' % SAMPLEQ,modelslopes)
np.savetxt(directorydataoutput + 'R2_1920-2080-FORCEDens_XGHG-XAER-LENS_%s_RANDOMSEED_20ens.txt' % SAMPLEQ,modelr2)
...
def netcdfLENS(lats,lons,var,directory,SAMPLEQ):
    ...
    name = 'LRP_FORCEDens_Maps_%s_20ens.nc' % SAMPLEQ
