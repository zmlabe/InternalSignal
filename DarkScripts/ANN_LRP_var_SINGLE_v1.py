"""
Train the model on the different LENS-SINGLE runs
...
for sis,singlesimulation in enumerate(datasetsingle):
    ...
    directoryfigure = '/Users/zlabe/Desktop/SINGLE/%s/' % variq
    ...
    def beginFinalPlot(YpredTrain,YpredTest,Ytrain,Ytest,testIndices,years,yearsObs,YpredObs):
        ...
        #              fontsize=5,
        #              color='gray')
        plt.savefig(directoryfigure+savefigName+'.png',
                    dpi=300)      
    ...
    ### Take LRP mean
    perclrpmean = np.nanmean(perclrp[:],axis=0)
    ...
    ### Save figure    
    if rm_ensemble_mean == True:
        plt.savefig(directoryfigure + 'PERC_%s_%s_%s_%s_ENSMEAN-REMOVED.png' % (variq,
                                                                                  monthlychoice,
                                                                                  reg_name,
                                                                                  dataset),dpi=300)
    else:
        plt.savefig(directoryfigure + 'PERC_%s_%s_%s_%s.png' % (variq,
                                                                              monthlychoice,
                                                                              reg_name,
                                                                              dataset),dpi=300)
    ...
    if years.max() == 2029:
        ...
        plt.savefig(directoryfigure + 'LRPsubplots_%s_%s_%s_%s_.png' % (variq,monthlychoice,reg_name,dataset),dpi=300)
    elif years.max() == 2080:
