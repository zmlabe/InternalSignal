"""
Plots LRP maps of uncertainty from ensembles with forced mean removed
...
### Set parameters
variables = [r'T2M']
datasets = [r'XGHG--ENS',r'XAER--ENS',r'LENS--ENS']
seasons = [r'annual']
...
directoryfigure = '/Users/zlabe/Desktop/SINGLE_v2.0/Histograms/rm_ensemble_mean/LRP/%s/' % variables[0]

### Read in LRP maps
data = Dataset(directorydata + 'LRP_FORCEDens_Maps_%s_%s_%s_20ens.nc' % (variables[0],seasons[0],SAMPLEQ))
...
for i in range(len(datasets)):
    ...
    ### Colorbar limits
    barlim = np.round(np.arange(0,0.4,0.1),2)
    ...
    cs = m.contourf(x,y,var,np.arange(0,0.3001,0.005),
                    extend='max')                
...
plt.savefig(directoryfigure + 'LRPmean_%s_%s_%s_FORCED.png' % (variables[0],
                                                        seasons[0],
                                                        SAMPLEQ),
                                                        dpi=300)
...
for i in range(len(datasets)):
    ...
    ### Colorbar limits
    barlim = np.round(np.arange(0,0.051,0.025),3)
    ...
    ### Make the plot continuous
    cs = m.contourf(x,y,var,np.arange(0,0.051,0.001),
                    extend='max')                
...
plt.tight_layout()
plt.subplots_adjust(bottom=0.17)
plt.savefig(directoryfigure + 'LRPstd_%s_%s_%s_FORCED.png' % (variables[0],
                                                        seasons[0],
                                                        SAMPLEQ),
                                                        dpi=300)
...
plt.tight_layout()
plt.subplots_adjust(bottom=0.17)
plt.savefig(directoryfigure + 'Percmean_%s_%s_%s_FORCED.png' % (variables[0],
                                                        seasons[0],
                                                        SAMPLEQ),
                                                        dpi=300)