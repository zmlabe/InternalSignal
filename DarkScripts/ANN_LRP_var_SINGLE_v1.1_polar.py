"""
Train the model on the different LENS-SINGLE runs for different seasons
...
for sis,singlesimulation in enumerate(datasetsingle):
    ...
    for seas in range(len(seasons)):
        ...
        if polregion == 'North':
            ...
            directoryfigure = '/Users/zlabe/Desktop/SINGLE_v1.1/Polar/%s/' % polregion 
        elif polregion == 'South':
            mapbase='spstere'
            latlower=-60
            reg_name = 'LowerAntarctic'
            lat_bounds,lon_bounds = UT.regions(reg_name)
            directoryfigure = '/Users/zlabe/Desktop/SINGLE_v1.1/Polar/%s/' % polregion 
        ...
        if years.max() == 2029:
            ...
            for i,yr in enumerate(range(0,lrp.shape[0],len(years)//5)):
                ...
                m = Basemap(projection=mapbase,boundinglat=latlower,lon_0=270,resolution='l',round=True,area_thresh=10000)
        ...
        if rm_ensemble_mean == False:
            if variq == 'U700':
                ...
                plt.yticks(np.arange(-110,20.25,1),map(str,np.round(np.arange(-110,20.25,1),2)),size=6)
                plt.xticks(np.arange(yearsall[sis].min(),2101,20),map(str,np.arange(yearsall[sis].min(),2101,20)),size=6)
                plt.xlim([yearsall[sis].min(),yearsall[sis].max()])   
