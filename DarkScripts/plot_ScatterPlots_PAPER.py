...
ax.tick_params('both',length=4,width=2,which='major',color='darkgrey',pad=1.1)

ax.fill_between(years,annual_minperc[0],annual_maxperc[0],color='deepskyblue',
                alpha=1,linewidth=0,clip_on=False)
plt.scatter(yearobs,obs_annual[0],s=12,color='crimson',clip_on=False,
            alpha=1,linewidths=0.5)
plt.plot(years,years,'-',color='w',linewidth=1,clip_on=False)
plt.plot(yearobs,lines_annual[0],linewidth=1.5,linestyle='--',dashes=(1,0.3),
          color='w')

# plt.xlabel(r'\textbf{ACTUAL YEAR}',fontsize=10,color='darkgrey')
plt.ylabel(r'\textbf{PREDICTED YEAR}',fontsize=10,color='darkgrey')
...
plt.text(1920,2074,r'\textbf{AER+}',fontsize=20,color='w',ha='left',va='center')
plt.text(2082,1921,r'\textbf{R$^{2}$=%s}' % np.round(r2_annual[0],2),fontsize=8,color='crimson',
          ha='right',va='center')
...
ax.fill_between(years,annual_minperc[1],annual_maxperc[1],color='deepskyblue',
                alpha=1,linewidth=0,clip_on=False,label=r'\textbf{Models [Training+Testing]}')
plt.scatter(yearobs,obs_annual[1],s=12,color='crimson',clip_on=False,
            alpha=1,linewidths=0.5,label=r'\textbf{Observations}')
plt.plot(years,years,'-',color='w',linewidth=1,clip_on=False)
...
le = plt.legend(shadow=False,fontsize=7,loc='upper center',
            ...
            labelspacing=0.2)
ccc = ['deepskyblue','crimson']
for counter,text in enumerate(le.get_texts()):
    text.set_color(ccc[counter]) 

# plt.text(2082,2084,r'\textbf{[b]}',fontsize=8,color='k',ha='right',va='center')
plt.text(1920,2074,r'\textbf{GHG+}',fontsize=20,color='w',ha='left',va='center')
plt.text(2082,1921,r'\textbf{R$^{2}$=%s}' % np.round(r2_annual[1],2),fontsize=8,color='crimson',
          ha='right',va='center')
...
plt.text(1920,2074,r'\textbf{ALL}',fontsize=20,color='w',ha='left',va='center')
plt.text(2082,1921,r'\textbf{R$^{2}$=%s}' % np.round(r2_annual[2],2),fontsize=8,color='crimson',
          ha='right',va='center')

plt.subplots_adjust(bottom=0.17)
plt.savefig(directoryfigure + 'ScatterPrediction_Annual_T2M_PAPER_DARK.png',
            dpi=600)
...
for i in range(len(seasons_model)):
    ...
    if any([i==0,i==2]):
        plt.ylabel(r'\textbf{PREDICTED YEAR}',fontsize=10,color='darkgrey')
    if i < 2:
        plt.title(r'\textbf{%s}' % title[i],color='w',fontsize=24)
    plt.xticks(np.arange(years.min(),2101,20),map(str,np.arange(years.min(),2101,20)),size=6)
    plt.yticks(np.arange(years.min(),2101,20),map(str,np.arange(years.min(),2101,20)),size=6)
    plt.xlim([years.min(),years.max()])   
    plt.ylim([years.min(),years.max()])
    ...
    plt.text(1920,2074,r'\textbf{%s}' % time[i],fontsize=15,color='darkgrey',ha='left',va='center')
    plt.text(2082,1921,r'\textbf{R$^{2}$=%s}' % np.round(r2all,2),fontsize=8,color='crimson',
              ha='right',va='center')

plt.tight_layout()
plt.savefig(directoryfigure + 'ScatterPrediction_Seasons_Subplot_T2M_DARK.png',
            dpi=600)