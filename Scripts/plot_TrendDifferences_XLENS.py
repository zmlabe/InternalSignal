"""
Plot maps of the trend differences between LENS-[XGHG+XAER]
...
datasetsingleq = np.repeat(['TOTAL--(GHG+AER)'],1)
...
# for count,i in enumerate(range(0,len(years),40)):
#     trendens_ghg[:,count,:,:] = calcTrend(ghgn[:,i:i+40,:,:])
#     trendens_aer[:,count,:,:] = calcTrend(aern[:,i:i+40,:,:])
#     trendens_lens[:,count,:,:] = calcTrend(lensn[:,i:i+40,:,:])

# ## Calculate difference in trends  
# difference = trend_lens - (trend_ghg + trend_aer)
# difference_all = trendens_lens - (trendens_ghg + trendens_aer)
# summ = trendens_ghg + trendens_aer
...
# stat,pvalue = UT.calc_indttest(summ,trendens_lens)

# ### Mask significane
# pvalue[np.isnan(pvalue)] = 0.
# pvalue[pvalue == 1.] = np.nan
# pvalue[pvalue == 0.] = 1.
...
label = r'\textbf{DIFFERENCE: T2M [$^{\circ}$C decade$^{-1}$]}'
...
for r in range(len(runs)):
    ...
    ax1 = plt.subplot(1,4,r+1)
    m = Basemap(projection='moll',lon_0=0,resolution='l',area_thresh=10000)
    circle = m.drawmapboundary(fill_color='k')
    circle.set_clip_on(False) 
    m.drawcoastlines(color='darkblue',linewidth=0.45)
...
###########################################################################
cbar_ax = fig.add_axes([0.32,0.17,0.4,0.03])                
...
plt.savefig(directoryfigure + 'DifferenceTrendPeriods_T2M_XGHG-XAER-LENS.png',dpi=300)
