...
# data,lats,lons=read_primary_dataset(dataset,lat_bounds=lat_bounds,lon_bounds=lon_bounds)
  
def read_obs_dataset(dataset_obs,lat_bounds=lat_bounds,lon_bounds=lon_bounds):
    data_obs,lats_obs,lons_obs = df.readFiles(dataset_obs)
    data_obs,lats_obs,lons_obs = df.getRegion(data_obs,lats_obs,lons_obs,
                                            lat_bounds,lon_bounds)
    print('our OBS dataset: ',dataset_obs,' is shaped',data_obs.shape)
    return data_obs,lats_obs,lons_obs

# data_obs,lats_obs,lons_obs = read_obs_dataset(dataset_obs,lat_bounds=lat_bounds,lon_bounds=lon_bounds)
...
#     '''function to delete data and data_obs since we have already sliced other variables from them.  Only run after segment_data and shape_era!!!
...
#         data_obs_all, lats_era, lons_era = read_era_dataset(dataset_era, lat_bounds, lon_bounds)
...
#             data, data_obs, data_cmip5, data_best = data_all, data_obs_all, data_cmip5_all, data_best_all
#             if rm_annual_mean == True:
#                 data, data_obs = remove_annual_mean(data, data_obs)
#                 data, data_best = remove_annual_mean(data, data_best)
#                 data_cmip5, ___ = remove_annual_mean(data_cmip5, data_obs)
...
#                 data_cmip5, ___ = remove_merid_mean(data_cmip5, data_obs)
#                 print('removed meridian mean')

#             if land_only == True:
#                 data, data_obs = land_only_segment(data, data_obs)
#                 data, data_best = land_only_segment(data, data_best)
#                 data_cmip5, ___ = land_only_segment(data_cmip5, data_obs)
#                 print('land only')     
