"""
Train the model on randomized labels (shuffle along LENS time dimension)
    ...
    for seas in range(len(seasons)):
        ...
        for isample in range(SAMPLEQ): 
            ...
            def read_primary_dataset(variq,dataset,lat_bounds=lat_bounds,lon_bounds=lon_bounds):
                ...
                temp = datar
                tempswitch = np.swapaxes(temp, 0, 1)
                np.random.shuffle(tempswitch)
                tempfinal = np.swapaxes(tempswitch,0,1)
                print('\n\n<<<<<<< SHUFFLED ARRAY FOR TESTING STATS ON TIME DIMENSION!!! >>>>>>\n\n')
...
np.savetxt(directorydataoutput + 'Slopes_20CRv3-SHUFFLE-LABELSv2_%s_RANDOMSEED_20ens.txt' % SAMPLEQ,modelslopes)
np.savetxt(directorydataoutput + 'R2_20CRv3-SHUFFLE-LABELSv2_%s_RANDOMSEED_20ens.txt' % SAMPLEQ,modelr2)
...
def netcdfLENS(lats,lons,var,directory,SAMPLEQ):
    ...
    name = 'LRP_Maps_%s_20ens_SHUFFLE-LABELSv2.nc' % SAMPLEQ
