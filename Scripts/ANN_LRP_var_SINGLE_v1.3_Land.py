...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep 23 09:24:02 2020
...
"""
Train the model on the different LENS-SINGLE runs for different seasons just 
over land [mask]
...
for sis,singlesimulation in enumerate(datasetsingle):
    ...
    for seas in range(len(seasons)):
        ...
        analyzer_outputscale=analyzer_output/np.nansum(analyzer_output,axis=1)[:,np.newaxis]   
        
        numLats = lats.shape[0]
        numLons = lons.shape[0]  
        lrpobs = analyzer_outputscale.reshape(np.shape(analyzer_outputscale)[0],numLats,numLons)*1000
        ...
        ### Append maps of lrp
        lrpsns.append(lrp)
        lrpobssns.append(lrpobs)
    lrpsns = np.asarray(lrpsns)
    lrpobssns = np.asarray(lrpobssns)
    #######################################################################
    #######################################################################
    #######################################################################
