...
ax.fill_between(years, min2tdghg, max2tdghg, facecolor='steelblue',
...
plt.savefig(directoryfigure + 'TimeSeries_T2M_XGHG-XAER-LENS-20CRv3.png',
            dpi=300)
...
plt.ylabel(r'\textbf{T2M [$^{\circ}$C]}',fontsize=10,color='dimgrey')
plt.yticks(np.arange(-10,101,0.2),map(str,np.round(np.arange(-10,101,0.2),2)),size=6)
plt.xticks(np.arange(1920,2080+1,20),map(str,np.arange(1920,2080+1,20)),size=6)
plt.xlim([1920,2020])   
plt.ylim([-0.6,1.4])
...
plt.savefig(directoryfigure + 'TimeSeries_T2M_XGHG-XAER-LENS-20CRv3_Zoom.png',
            dpi=300)