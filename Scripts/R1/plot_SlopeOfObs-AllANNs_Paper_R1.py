"""
Plots histograms of slope of observations for paper showing possible ANNs with
...
def readData(directorydata,SAMPLEQ,l2,epochs):
    ### Read in R2 data
    filename_R2 = 'Slopes_20CRv3-Obs_XGHG-XAER-LENS_%s_RANDOMSEED_20ens_R1-Trials_L2-%s_epochs-%s.txt' % (SAMPLEQ,l2,epochs)
    ...
    np.savetxt(directorydata + 'Slopes_20CRv3-Obs_XGHG-XAER-LENS_%s_RANDOMSEED_Medians_20ens_R1-Trials_L2-%s_epochs-%s.txt' % (SAMPLEQ,l2,epochs),
               mediansall)
...
for s in range(len(l2)):
    ...
    ### Plot perfect slope
    plt.axvline(x=1,color='k',linewidth=1,linestyle='--',dashes=(1,0.3),
                zorder=10)
    ...
    plt.text(1.5,0.8,r'\textbf{[%s]}' % letters[s],color='dimgrey',fontsize=8,
                          rotation=0,ha='center',va='center')
        
    plt.xlim([-1.2,1.5])   
    plt.ylim([0,0.8])
    
plt.savefig(directoryfigure + 'HistogramSlopeOfObs-AllANNs_PAPER.png',dpi=600)