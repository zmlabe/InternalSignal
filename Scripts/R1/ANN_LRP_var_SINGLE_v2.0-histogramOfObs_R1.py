...
while trying different hyperparameter combinations
...
for sis,singlesimulation in enumerate(datasetsingle):
    for seas in range(len(seasons)):
        ...
        for isample in range(SAMPLEQ): 
            ...
            r_spear = sts.spearmanr(obsactual,obspredic)[0]
            
            ### Append slopes
            valslopes.append(slopeobs)
            valrr.append(r_spear)
...
##############################################################################
##############################################################################
def netcdfLENS(lats,lons,var,directory,SAMPLEQ,ridge_penalty,iterations):
    print('\n>>> Using netcdf4LENS function!')
    
    from netCDF4 import Dataset
    import numpy as np
    
    name = 'LRP_YearlyMaps_%s_20ens_T2M_annual_R1-Trials_L2-%s_epochs-%s.nc' % (SAMPLEQ,ridge_penalty[0],iterations[0])
...
netcdfLENS(lats,lons,lrpmapsallarray,directorydataoutput,SAMPLEQ,ridge_penalty,iterations)